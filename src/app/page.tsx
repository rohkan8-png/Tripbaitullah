...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const timer1 = animateCounter(setBulanIni, 50, 2000)
    const timer2 = animateCounter(setBulanDepan, 25, 2000)
    const timer3 = animateCounter(setTotalPaket, 100, 2000)
  ...
  const fetchPackages = async (location: string = '') => {
    try {
      ...
      const response = await fetch(`/api/packages?limit=6${locationParam}`)
                  ...
                  {[...Array(8)].map((_, index) => (
                    <div key={index} className="bg-card border border-border rounded-2xl overflow-hidden animate-pulse">
                      ...
                      <div className="p-3">
                        ...
                        <div className="h-3 bg-muted rounded-lg mb-1.5"></div>
              ...
              {loadingTravels ? (
                <div className="overflow-x-auto -mx-3 px-3 md:mx-0 md:px-0">
                  <div className="flex gap-3 pb-2">
                    {[...Array(6)].map((_, index) => (
                      <div key={index} className="flex-shrink-0 w-[260px] md:w-[300px] bg-card border border-border rounded-2xl p-4 animate-pulse">
                        <div className="flex items-center gap-3">
                          <div className="w-12 h-12 md:w-14 md:h-14 bg-muted rounded-xl"></div>
            ...
            <section>
              ...
              {loadingVideos ? (
                <div className="overflow-x-auto -mx-3 px-3 md:mx-0 md:px-0">
                  <div className="flex gap-3 pb-2">
                    {[...Array(4)].map((_, index) => (
                      <div key={index} className="flex-shrink-0 w-[calc(50%-6px)] md:w-[320px] bg-card border border-border rounded-2xl overflow-hidden animate-pulse">
